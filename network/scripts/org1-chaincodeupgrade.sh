...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.settle.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n setcc -v ${VERSION} -c '{"Args":["initLedger"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')"
  res=$?
  set +x
  cat log.txt
...
echo
echo "========= Finished adding Org2 to your first network! ========= "
echo
